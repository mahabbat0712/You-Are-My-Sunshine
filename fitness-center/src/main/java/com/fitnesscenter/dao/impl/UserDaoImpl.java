...
public class UserDaoImpl extends AbstractDao<User, Long> implements UserDao {
    ...
    public boolean removeRoleFromUser(Long userId, Integer roleId) {
        ...
        return update(REMOVE_ROLE_FROM_USER_SQL, userId, roleId) > 0;
    }
    
    @Override
    public Optional<ClientProfile> findClientProfileByUserId(Long userId) {
        logger.debug("Finding client profile for user: {}", userId);
        String sql = "SELECT cp.id, cp.user_id, cp.date_of_birth, cp.health_notes, " +
                "cp.completed_cycles, cp.account_type_id, at.name as account_type_name, " +
                "at.description as account_type_description " +
                "FROM client_profiles cp " +
                "LEFT JOIN account_types at ON cp.account_type_id = at.id " +
                "WHERE cp.user_id = ?";
        
        try (Connection conn = databaseManager.getConnection();
             var stmt = prepareStatement(conn, sql, userId)) {
            
            ResultSet rs = stmt.executeQuery();
            if (rs.next()) {
                return Optional.of(mapClientProfileRow(rs));
            }
            return Optional.empty();
        } catch (SQLException e) {
            logger.error("Error finding client profile for user: {}", userId, e);
            throw new RuntimeException("Error finding client profile", e);
        }
    }
    ...
    public ClientProfile saveClientProfile(ClientProfile profile) {
        if (profile.getId() == null) {
            return insertClientProfile(profile);
        } else {
            return updateClientProfile(profile);
        }
    }
    
    private ClientProfile insertClientProfile(ClientProfile profile) {
        logger.debug("Inserting new client profile: {}", profile);
        String sql = "INSERT INTO client_profiles (user_id, date_of_birth, health_notes, " +
                "completed_cycles, account_type_id) VALUES (?, ?, ?, ?, ?)";
        
        Optional<Long> generatedId = updateAndGetGeneratedKey(sql,
                profile.getUser().getId(),
                profile.getDateOfBirth(),
                profile.getHealthNotes(),
                profile.getCompletedCycles(),
                profile.getAccountType() != null ? profile.getAccountType().getId() : null
        );
        
        if (generatedId.isPresent()) {
            profile.setId(generatedId.get());
            return profile;
        } else {
            throw new RuntimeException("Failed to insert client profile: " + profile);
        }
    }
    
    private ClientProfile updateClientProfile(ClientProfile profile) {
        logger.debug("Updating client profile: {}", profile);
        String sql = "UPDATE client_profiles SET date_of_birth = ?, health_notes = ?, " +
                "completed_cycles = ?, account_type_id = ? WHERE id = ?";
        
        int rowsUpdated = update(sql,
                profile.getDateOfBirth(),
                profile.getHealthNotes(),
                profile.getCompletedCycles(),
                profile.getAccountType() != null ? profile.getAccountType().getId() : null,
        ...
        if (rowsUpdated > 0) {
            ...
            throw new RuntimeException("Failed to update client profile: " + profile);
        }
    }
    
    private ClientProfile mapClientProfileRow(ResultSet rs) throws SQLException {
        ...
        profile.setCompletedCycles(rs.getInt("completed_cycles"));
        
        // Set account type if present
        if (rs.getObject("account_type_id") != null) {
            var accountType = new com.fitnesscenter.model.AccountType();
            accountType.setId(rs.getInt("account_type_id"));
            accountType.setName(rs.getString("account_type_name"));
            accountType.setDescription(rs.getString("account_type_description"));
